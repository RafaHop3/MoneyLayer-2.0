...
import os, stripe, psycopg2, random, smtplib
...
def send_email(to_email, code):
    try:
        user = 'theorbesystems@gmail.com'
        password = 'ways cjqv mtmr fwhs'
        msg = MIMEText(f'Seu código de acesso MoneyLayer é: {code}')
        msg['Subject'] = '🔐 MoneyLayer - Seu Código de Acesso'
        msg['From'] = user
        msg['To'] = to_email
...
<head>
    ...
    <style>
        body { font-family: 'Arial', sans-serif; background: #e9ecef; display: flex; align-items: center; justify-content: center; min-height: 100vh; margin: 0; }
        .main-card { display: flex; width: 800px; background: white; box-shadow: 0 10px 30px rgba(0,0,0,0.2); border-radius: 12px; overflow: hidden; flex-wrap: wrap; }
        .left { flex: 1; padding: 40px; background: #fff; border-right: 1px solid #ddd; min-width: 300px; }
        .right { flex: 1; padding: 40px; background: #f8f9fa; text-align: center; min-width: 300px; }
    ...
    <div class="main-card">
        ...
        <div class="right">
            ...
            <form action="/create-checkout-session" method="POST">
                <button type="submit" class="btn btn-stripe">Pagar com Stripe</button>
            </form>
        </div>
    </div>
</body>
...
<body>
    <div class="terminal">
        ...
        <p>> BANCO: <span class="status">SECURE (SSL)</span></p>
        <p>> INTERESSE SOCIAL: <span class="status">ATIVO</span></p>
        <br>
        <p>=========================================</p>
...
def gerar_token():
    ...
    try:
        ...
        # MOSTRA NA TELA TAMBÉM (DUPLA SEGURANÇA)
        return f"""
        <body style='background:#222; text-align:center; color:white; font-family:sans-serif; padding-top:100px;'>
            ...
            <p>O código foi enviado para <strong>{email}</strong></p>
...
def create_checkout_session():
    try:
        session = stripe.checkout.Session.create(
            ...
            mode='payment',
            success_url=request.host_url + 'audit_redirect',
